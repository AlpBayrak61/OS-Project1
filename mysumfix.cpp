...
static inline void arriveAndWait(int id, int m, volatile int* phase_ptr, volatile int* arrived_array){
    ...
    void worker(int id, int n, int m, long long* arr0, long long* volatile int* counter_ptr, volatile int* arrived_array){
        int chunk = (n + m - 1)/ m;
        int start = id * chunk;
        ...
        arrive(id,m,counter_ptr, arrived_array);
        }
